...
grid_pcp2SWAT <- function(
  ...
  # verificar el shape
  if(!class(Subbasins) == 'SpatialPolygonsDataFrame') stop('subs shuold be a SpatialPolygonsDataFrame object')
...
grid_tas2SWAT <- function(
  ...
  # verificar el shape
  if(!class(Subbasins) == 'SpatialPolygonsDataFrame') stop('subs shuld be a be a SpatialPolygonsDataFrame object')

  # verificar la proyecicon de subs con grid
  if(sp::is.projected(Subbasins)){
    stop('Error: Subs dont have projection')
  }  else {
